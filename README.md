...
Redis 是一个集中式缓存工具，用来管理：共享会话和系统缓存的，如果你已安装 Redis，可以忽略这个步骤。

由于 Windows 版本的 Redis 安装不太方便，所有我们提供了该版本的安装文件和安装脚本。

下载 Redis 打开 <https://gitee.com/thinkgem/jeesite4-cloud/attach_files> 找到 `Redis-x64-4.0.2.2.zip` 下载文件。
...
下载 Nacos 打开 <https://gitee.com/thinkgem/jeesite4-cloud/attach_files> 找到 `nacos-server-1.2.1.zip` 下载文件。
