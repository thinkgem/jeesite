 ...
 * Copyright (c) 2008, Tony Tomov, tony@trirand.com, ThinkGem
...
$.extend($.jgrid,{
	...
	jqID : function(sid){
		...
		return String(sid).replace(/[!"#$%&'()*+,.\/:; <=>?@\[\\\]\^`{|}~]/g,"_"); // ThinkGem 修改将特殊字符替换为“下划线”
	},
	guid : 1,
	uidPref: 'jqg',
	...
	getAccessor : function(obj, expr) {
		...
		// ThinkGem 支持 colModel.name 设置 Map，如：optionMap[\'dictType\']
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		formatCol = function (pos, rowInd, tv, rawObject, rowId, rdata){
			...
			if(nm=="actions") {clas = 'actions ' + (clas ? clas : ''); } // ThinkGem 操作列特有的td属性
			//if(nm=="cb") { clas = 'cb ' + (clas ? clas : ''); } // ThinkGem 复选框列增加 cb 类，去掉省略号样式（有了，先不需要了）
			...
			result += (clas !== undefined ? (" class=\""+clas+"\"") :"") + ((cm.title && tv) ? (
				// ThinkGem 当 autoencode 为 false 的时候，如果 title 未 html 编码导致 html 显示不完整
		...
		addXmlData = function (xml,t, rcnt, more, adjust) {
			...
			if(gxml && gl){
				...
				while (j<gl) {
					...
					j++;
//					if(ir===rn) {break;}
					if(ts.p.datatype === "local" && ir===rn) {break;} // ThinkGem 支持 jqgrid 树表一级数据分页。
				}
			}
		...
		addJSONData = function(data,t, rcnt, more, adjust) {
			...
			while (i<len) {
				...
				if(locdata || ts.p.treeGrid===true) {
					...
					rd[ts.p.localReader.id] = rd[locid]; // 本地树表数据，如果 colModel 里没有定义id列，则会报找不到列数据的问题  ThinkGem
					ts.p.data.push(rd);
...
//				if(ir===rn) { break; }
			...
			if(ts.p.gridview === true ) {
				...
				if(ts.p.grouping) {
					...
					$("#"+$.jgrid.jqID(ts.p.id)+" tbody:first").append(rowData.join('')); // ThinkGem 恢复被注释的代码，下面这一行不支持ie8
					//ts.firstElementChild.innerHTML += rowData.join(''); // append to innerHTML of tbody which contains the first row (.jqgfirstrow)
					ts.grid.cols = ts.rows[0].cells; // update cached first row
				}
			}
...
//			recordsperpage = parseInt(ts.p.rowNum,10),
			recordsperpage = parseInt(ts.p.rowNum == -1 ? 5000 : ts.p.rowNum,10), // 如果设置为 -1 则给一个默认值 5000，否则会显示数据不全 ThinkGem
		...
		if(ts.p.grouping===true) {
			ts.p.scroll = false;
//			ts.p.rownumbers = false;  // ThinkGem 去掉默认分组情况下不显示行号
		...
		if(this.p.multiselect) {
			...
			this.p.colModel.unshift({name:'cb',width:$.jgrid.cell_width ? ts.p.multiselectWidth+ts.p.cellLayout : ts.p.multiselectWidth,sortable:false,resizable:false,hidedlg:true,search:false,align:'center',fixed:true,classes:'cb'}); // ThinkGem 复选框列增加 cb 类，去掉省略号样式
		}
...
//			thead += "<th id='"+ts.p.id+"_"+ts.p.colModel[i].name+"' role='columnheader' class='ui-state-default ui-th-column ui-th-"+dir+"'"+ tooltip+">";// ThinkGem classes
			thead += "<th id='"+ts.p.id+"_"+ts.p.colModel[i].name+"' role='columnheader' class='ui-state-default ui-th-column ui-th-"+dir+" "+(ts.p.colModel[i].classes||'')+"'"+ tooltip+">";// ThinkGem 表头支持 classes 设定
		...
		imgs = null;
		$(this).append(thead);
//		$(this).append(thead).find("#jqgh_"+ts.p.id+"_rn").width(ts.p.rownumWidth); // 修正多表头情况下，有一点点错位问题 ThinkGem，暂时无问题，取消设定，如果表头上加内容会不居中
...
//			firstr += "<td role='gridcell' style='height:0px;width:"+w+"px;"+hdcol+"'></td>";
			...
			if ($(e.target).hasClass('ui-jqgrid-resize-disable')) { return; } // 禁用已经禁用的列拖拽 ThinkGem
...
//						// ThinkGem 取消，点击行的时候选中行，只有点击复选框的是再选中行
...
//			.append($('<div style="position:relative;'+(isMSIE && $.jgrid.msiever() < 8 ? "height:0.01%;" : "")+'"></div>').append('<div></div>').append(this))
			.append(this) // ThinkGem 注释注释掉上方，因为打开会出现：数据少的情况下，鼠标放到更多按钮上的时候出现横向和竖向滚动条。
		...
		this.each(function() {
			...
			if( $t.p.altRows === true && success ) {
				...
				$($t.rows).each(function(i){
					if(i % 2 === 0) { $(this).addClass(cn); } // ThinkGem 删除后的斑马线计算 i % 2 === 1  改为  i % 2 === 0
...
//							title = this.title ? {"title":$.jgrid.stripHtml(vl)} : {};
							title = this.title ? {"title":$.jgrid.stripHtml(vl=='&#160;'?'':vl)} : {};// ThinkGem title显示&#160;修正
							if(t.p.treeGrid===true && nm === t.p.ExpandColumn) {
	...
	addRowData : function(rowid,rdata,pos,src) {
		...
		}catch(e){}// ThinkGem 加上 try catch 修正IE8下脚本错误。
		...
		if(rdata) {
			...
			this.each(function() {
				...
				if( t.p.altRows === true && !aradd) {
					if (pos === "last") {
						if ((t.rows.length-1)%2 === 0)  {$(t.rows[t.rows.length-1]).addClass(cn);} // ThinkGem 增行后的斑马线计算  (t.rows.length-1)%2 === 1 改为 (t.rows.length-1)%2 === 0
					} else {
						$(t.rows).each(function(i){
...
//						$("tr.footrow td:eq("+i+")",t.grid.fsDiv).html(vl).attr(title); // ThinkGem 新增底部冻结行的时候，设置底部值。
						// ThinkGem 多个grid情况下，修正底部统计行错位问题
						$("tr.footrow td:eq("+i+")",$(t.grid.sDiv)).html(vl).attr(title);
						$("tr.footrow td:eq("+i+")",$(t.grid.fsDiv)).html(vl).attr(title); // ThinkGem 新增底部冻结行的时候，设置底部值。
						$("tr.footrow td:eq("+i+")",$(t.grid.fsDivRight)).html(vl).attr(title); // ThinkGem 新增底部冻结行的时候，设置底部值。
						success = true;
					}
				} else if(action === "get") {
	...
	showHideCol : function(colname,show,inLabel) { // 新增 inLabel 参数，当列名相同时，使用表头名+表头标题作为判断条件，解决name相同问题 ThinkGem
		return this.each(function() {
			...
			$(this.p.colModel).each(function(i) {
				...
				// 新增 inLabel 参数，当列名相同时，使用表头名+表头标题作为判断条件，解决name相同问题 ThinkGem
...
//				$($t).jqGrid("setGridWidth",$t.p.shrinkToFit === true ? $t.p.tblwidth : $t.p.width ); // 不刷新宽度，防止排序后闪眼 ThinkGem
			}
			if( gh )  {
	...
	hideCol : function (colname,inLabel) {// 新增 inLabel 参数 ThinkGem
		return this.each(function(){$(this).jqGrid("showHideCol",colname,"none",inLabel);});
	},
	showCol : function(colname,inLabel) {// 新增 inLabel 参数 ThinkGem
	...
	setGridHeight : function (nh) {
		return this.each(function (){
			...
			if($t.p.frozenColumns === true){
				...
				$('#'+$.jgrid.jqID($t.p.id)+"__frozen").parent().height(bDiv.height()-($.jgrid.scrollsize)); // ThinkGem 修正IE下高度计算错误
				$('#'+$.jgrid.jqID($t.p.id)+"__frozen_right").parent().height(bDiv.height()-($.jgrid.scrollsize)); // ThinkGem 修正IE下高度计算错误
				// 如果屏幕过小（宽度小于600）或者没有水平滚动条了，则没必要再显示锁定列了 ThinkGem
...
//Helper functions
	...
	createModal : function(aIDs, content, p, insertSelector, posSelector, appendsel, css) {
		...
		if ( $.fn.jqm && p.jqModal === true) {
			...
			coord.top = p.top+"px";
//			coord.left = p.left;
			coord.left = p.left + $(window).width()/2-p.width/2; // ThinkGem 调整到屏幕中间
...
//					if(!options.cols) { $(elem).css({width:"99%",height:"100%"});} // ThinkGem 兼容firefox
			...
			case "radio":
			case "checkbox":
				// ThinkGem 新增编辑单元格checkbox支持数组类型（配合选项：items,itemLabel,itemValue）
				if(options.items) {
			...
			case "select" :
				...
				$(elem).css({width:"99%"}); // ThinkGem 行编辑控件宽度
				var msl, ovm = [];
				...
				// ThinkGem 新增编辑单元格select支持数组类型（配合选项：items,itemLabel,itemValue）
...
//						if(!options.size) { $(elem).css({width:"99.9%"}); }
						if(!options.size) { $(elem).css({width:"99%"}); } // ThinkGem 顶满单元格
			...
			for (i = 0; i < cml; i++) {
				...
				if (iCol >= 0) {
					...
					$colHeader = $('<th>').attr({role: "columnheader"})
						.addClass("ui-state-default ui-th-column-header ui-th-"+ts.p.direction+" "+(cghi.classes||''))// ThinkGem classes
				...
				$('td[role="gridcell"]',ind).each( function(i) {
					...
					if ( nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn') {
						...
						if(cm[i].editable===true) {
							...
							$(this).css('overflow','visible');// ThinkGem add td溢出显示，增强 验证，防止验证信息被遮挡
							// ThinkGem 添加默认参数，如checkbox或select多选情况下，不选择则提交该空值，而不提交null
...
//							}
							if (e.keyCode === 13 && !e.shiftKey) {
								...
								// ThinkGem 编辑行，回车时间修改为获取下一个元素焦点， 修改前
...
//								}
					...
					// 编辑时，表单控件获取焦点的时候自动选择当前行 ThinkGem
					if (!$t.p.multiboxonly) {
	...
	saveRow : function(rowid, successfunc, url, extraparam, aftersavefunc,errorfunc, afterrestorefunc) {
		...
		if (editable==="1") {
			...
			$('td[role="gridcell"]',ind).each(function(i) {
				...
				$(this).css('overflow','hidden');// ThinkGem add td溢出显示，增强 验证，防止验证信息被遮挡,,反向方法，取消显示，改为隐藏
			});
			if (cv[0] === false){
				try {
	...
	addRow : function ( p ) {
		p = $.extend(true, {
			...
			initdata : {},
			position : "first",
			src : null, // jqgrid addRow 支持插入到指定位置。  ThinkGem
			useDefValues : true,
			useFormatter : false,
...
//						p.initdata[this.name] = tmp;
						p.initdata[this.name.replace('[','.').replace(/\'/g,'').replace(']','')] = tmp; // 2018-05-15 修改name为map时，初始值无效问题。 ThinkGem
					}
				});
			}
//			$($t).jqGrid('addRowData', p.rowID, p.initdata, p.position);
			$($t).jqGrid('addRowData', p.rowID, p.initdata, p.position, p.src); // jqgrid addRow 支持插入到指定位置。 ThinkGem
...
$.jgrid.extend({
	setTreeNode : function(i, len){
		return this.each(function(){
			...
			ldat, lf;
			// ThinkGem 如果没有子节点，则将该节点转换为子节点图标
			if (i == len){
...
//			$t.p.multiselect = false; // 不限制树表添加复选框 ThinkGem
			$t.p.rowList = [];
			$t.p.expColInd = 0;
	...
	expandNode : function(rc) {
		return this.each(function(){
			...
			if($.isFunction(this.p.onExpandNode)) {this.p.onExpandNode(rc);} // ThinkGem 添加节点展开事件 onExpandNode
		});
	},
	collapseNode : function(rc) {
		return this.each(function(){
			...
			if($.isFunction(this.p.onCollapseNode)) {this.p.onCollapseNode(rc);} // ThinkGem 添加节点折叠事件 onCollapseNode
		});
	},
