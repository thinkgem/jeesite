...
public class SourceRelation {
	...
	public SourceRelation(Relation relation, boolean outgoing) {
		...
		this.finalSourceTable = new SourceTable(
				(ERTable) getFinalSourceTableView());
		this.finalTargetTable = new SourceTable(
				(ERTable) getFinalTargetTableView());
	}

	public SourceRelation(SourceColumn sourceColumn, Relation relation,
			boolean outgoing) {
	...
	public String getRelationType() {
		if (isOutgoing()) {
			if (CHILD_CARDINALITY_1.equals(relation.getChildCardinality())
					|| CHILD_CARDINALITY_0_OR_1.equals(relation
							.getChildCardinality())) {
	...
	public boolean isMappedBy() {
		...
		if (RELATION_TYPE_MANY_TO_MANY.equals(getRelationType())) {
			...
			if (!joinTable.getExpandedColumns().get(0).getRelationList().get(0)
					.getSourceTableView().getPhysicalName().equals(tableName)) {
				mappedBy = true;
			}
		}
		return mappedBy;
	...
	public String getTargetInstance() {
		...
		if (relation.getSource() == relation.getTarget()) {
			...
			if (RELATION_TYPE_ONE_TO_MANY.equals(getRelationType())) {
				targetUpperInstance = getFinalTargetTable()
						.getEntityClassName() + "List";
			} else if (RELATION_TYPE_MANY_TO_MANY.equals(getRelationType())) {
				targetUpperInstance = getFinalTargetTable()
						.getEntityClassName() + "List";
			} else {
				targetUpperInstance = StringUtils.upperFirst(getJoinColumn()
						.replace("_id", ""));
			}
		}
	...
	public String getJoinColumn() {
		String joinColumn = "";
		if (RELATION_TYPE_MANY_TO_ONE.equals(getRelationType())
				|| RELATION_TYPE_MANY_TO_MANY.equals(getRelationType())) {
			joinColumn = relation.getForeignKeyColumns().get(0)
					.getPhysicalName();
		}
		return joinColumn;
	}
	...
	public String getInverseJoinColumn() {
		...
		String tableName = relation.getSourceTableView().getPhysicalName();
		if (RELATION_TYPE_MANY_TO_MANY.equals(getRelationType())) {
			for (Relation r : relation.getTargetTableView()
					.getIncomingRelations()) {
				if (!r.getSourceTableView().getPhysicalName().equals(tableName)) {
					inverseJoinColumn = r.getForeignKeyColumns().get(0)
							.getPhysicalName();
				}
			}
		}
	...
	public boolean isTargetNotNull() {
		return CHILD_CARDINALITY_1.equals(relation.getChildCardinality())
				|| CHILD_CARDINALITY_1_OR_n.equals(relation
						.getChildCardinality());
	}
		...
		if (RELATION_TYPE_MANY_TO_MANY.equals(getRelationType())) {
			...
			for (NormalColumn normalColumn : joinTable.getExpandedColumns()) {
				for (Relation r : normalColumn.getRelationList()) {
					if (!r.getSourceTableView().getPhysicalName()
							.equals(tableName)) {
